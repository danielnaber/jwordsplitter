...
java -cp jwordsplitter-x.y.jar de.abelssoft.wordtools.jwordsplitter.converter.ExportDict /de/abelssoft/wordsGerman.ser

To serialize a text dictionary (one word per line) to a binary format
so it can be used by jWordSplitter, use this command:
java -cp jwordsplitter-x.y.jar de.abelssoft.wordtools.jwordsplitter.converter.SerializeDict textDict textDict.ser
